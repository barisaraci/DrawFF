...
class ProfileViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PostIdentifier", for: indexPath) as! ProfileViewCell
        ...
        cell.labelInfo.text = "You posted this image \(timeText) ago"
    ...
    func loadPosts(type: Int) {
        ...
        let json: [String: Any] = ["userId": userId, "postDate": postDate, "type": type, "isProfile": 1]
    ...
    func processResponse(result: [String: Any], type: Int) {
        ...
        if (posts.count == 0) {
            ...
            if (type == 1) { print(result) }
            DispatchQueue.main.async {
                self.labelUsername.text = "@\((self.posts.first! as Post).username)"
                if self.refreshControl.isRefreshing { self.refreshControl.endRefreshing() }
